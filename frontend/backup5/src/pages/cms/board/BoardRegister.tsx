...
const BoardFormPage: React.FC = () => {
  ...
  */
  const handleSave = async () => {
    //! ---------------------------------- [251015] 수정 ----------------------------------
    if (!boardNum.trim()) { //! [251015] 게시판 번호 필수 입력 검증
      alert("게시판 번호를 입력하세요. (예: 01, 02)");
      return;
    }
    ...
    try {
      ...
      params.append("boardContent", content);   // 본문 내용
      params.append("boardUse", usable ? "Y" : "N"); // 사용 여부
      ...
      // 4️⃣ 서버로 전송
      const res = await api.post("/api/cms/boards", params, config);

      // ✅ success가 false일 때만 실패로 간주
      if (res.data.success === false) {
        alert(res.data.message || "등록 실패 (중복 또는 서버 오류)");
        return;
      }
