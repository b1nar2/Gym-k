// [1] React 훅과 라우터 기능 불러오기
import React, { useEffect, useState } from "react"; // [1-1] React 훅
import { useParams, useNavigate } from "react-router-dom";  // [1-2] 라우터 훅

// [2] API 함수 불러오기
import { fetchFacilityById } from "../../../api/facilityApi"; // [2-1] 시설 단건조회
import { createReservation } from "../../../api/reservationApi"; // [2-2] 예약 등록

// [3] AuthContext 불러오기 → 로그인 정보 사용
import { useAuth } from "../../../auth/useAuth"; // [3-1]

// [2-1] MUI 컴포넌트 불러오기
import Box from '@mui/material/Box';  // *[251013] MUI 레이아웃 박스
import Typography from '@mui/material/Typography'; // *[251013] MUI 타이포그래피 텍스트
import TextField from '@mui/material/TextField'; // *[251013] MUI 입력창
import Button from '@mui/material/Button'; // *[251013] MUI 버튼
import Select from '@mui/material/Select'; // *[251013] MUI 셀렉트박스
import MenuItem from '@mui/material/MenuItem'; // *[251013] MUI 메뉴아이템
import InputLabel from '@mui/material/InputLabel'; // *[251013] MUI 입력레이블
import FormControl from '@mui/material/FormControl'; // *[251013] MUI 폼 컨트롤
...
export default function ReservationForm() {
  ...
  useEffect(() => {
    if (facility) {
      ...
      if (hours > 0) {
        setTotalMoney(hours * facility.facilityMoney);
      } else {
        setTotalMoney(0);
      }
    }
  ...
  // [10] 화면 렌더링
  return (
    <Box sx={{ p: 6, maxWidth: 720, mx: "auto" }}> {/* *[251013] MUI 전체 컨테이너, 최대 너비, 중앙 정렬 */}
      <Typography variant="h5" fontWeight="bold" mb={4}> {/* *[251013] MUI 제목, 굵게, 아래 여백 */}
        예약 신청
      </Typography>

      <Box mb={4}> {/* *[251013] MUI 정보 영역 아래 여백 */}
        <Typography variant="h6">{facility.facilityName}</Typography> {/* *[251013] 시설명 크고 볼드 */}
      ...
      <Box mb={4}>
        <InputLabel required>대표자</InputLabel> {/* *[251013] MUI 라벨 */}
        <TextField
          value={authState.memberName || ""}
          InputProps={{ readOnly: true }}
          fullWidth
          sx={{ bgcolor: "#F5F5F5" }} // *[251013] MUI 읽기전용 입력 필드 배경색
        />
      </Box>
        ...
        <Box sx={{ flex: 1 }}>
          ...
          <Select value={startHour} onChange={(e) => setStartHour(e.target.value)} fullWidth>
            {Array.from({ length: 13 }, (_, i) => 9 + i).map((h) => (
              <MenuItem key={h} value={String(h)}>{h}:00</MenuItem>
            ))}
          </Select>
        </Box>
        <Box sx={{ flex: 1 }}>
      ...
      <Button variant="contained" color="primary" onClick={handleSubmit} fullWidth sx={{ py: 1.5 }}>
        결제하기
      </Button>
    </Box>
  );
