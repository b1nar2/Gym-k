...
const BoardFormPage: React.FC = () => { // 함수형 컴포넌트 선언
  ...
  // 파일 선택 핸들러는 사용하지 않음(파일 업로드는 FileUploadInput으로 처리)
  ...
  // UI
  return ( // 화면 렌더링 시작
    <div className="p-8 bg-white rounded shadow-md max-w-3xl mx-auto">{/* 컨테이너 */}
      <h2 className="text-2xl font-bold mb-6">{isEditMode ? "게시판 수정" : "게시판 등록"}</h2>
      ...
      <div style={{ marginBottom: 12 }}> {/* 번호 입력 블록 */}
        ...
        <input
          type="text"
          value={boardNum}
          onChange={(e) => setBoardNum(e.target.value.replace(/[^0-9]/g, ""))}
          maxLength={2}
          placeholder="예: 01, 02"
  ...
  />
      ...
      <div className="flex justify-between">
        <button onClick={handleBack} className="button-secondary">목록</button>
        <button onClick={handleSave} className="button-primary">{isEditMode ? "수정" : "저장"}</button>
      </div>
    </div>
  ); // 렌더링 종료
};
