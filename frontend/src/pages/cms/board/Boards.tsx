...
export default function Boards() {
  ...
  return (
    <div>
      ...
      {/* 상단 검색/등록 UI (CmsUserList의 filter-search-row 및 버튼 스타일 적용) */}
      <div className="bg-white border border-slate-200 p-5 rounded-lg shadow-sm mb-5">
        {/* 기존: <div style={{ display: 'flex', marginBottom: 10, alignItems: 'center', gap: 8 }}> */}
        <div className="filter-search-row"> {/* filter-search-row 적용 */}
          <input
            type="text"
            placeholder="검색어 입력"
            value={keyword}
            ...
            // 기존: className="form-input" style={{ width: 180 }} 
            className="form-input filter-input w-full" // filter-input w-full 적용 
          />

          {/* 검색 버튼 */}
          {/* 기존: <button className="button-secondary" onClick={fetchBoards}>검색</button> */}
          <button className="common-button-style" onClick={fetchBoards}>검색</button> {/* common-button-style 적용 */}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}> 
        {/* 등록 버튼 */}
          {/* 기존: <button className="button-primary" onClick={handleRegister}>등록</button> */}
          <button className="primary-button-style" onClick={handleRegister}>등록</button> {/* primary-button-style 적용 */}
      </div>
            ...
            {pagedBoards.length === 0 ? (
              ...
              pagedBoards.map(b => (
                <tr key={b.boardNum}>
                  ...
                  <td>
                    <a
                      // * [251025] ${b.boardNum} → ${b.boardId}
                      href={`/cms/boards/${b.boardId}/posts`}
                      onClick={e => {
                        e.preventDefault();
                        navigate(`/cms/boards/${b.boardId}/posts`); // * [251025] ${b.boardNum} → ${b.boardId}
                      }}
                  ...
                  <td>
                    <div className="table-actions">
                      <button className="edit" onClick={() => handleEdit(b.boardId)}>편집</button>
                    </div>
                  </td>
