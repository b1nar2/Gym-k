...
public class CardServiceImpl implements CardService {
    ...
    */
    ...
    // ==============================[251009] 대표카드 및 나머지 카드조회  ==============================
    @Override
    @Transactional(readOnly = true)
    public List<CardResponse> getMainCards(String memberId) {
        return cardMapper.selectMainCards(memberId)
                         .stream().map(this::toResp)
                         .collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CardResponse> getSubCards(String memberId) {
        return cardMapper.selectSubCards(memberId)
                         .stream().map(this::toResp)
                         .collect(Collectors.toList());
    }
}
