...
public class FileController {
    ...
    // ----------- [251010] 수정: 실제 yml 키(fileUploadPath)와 일치시킴 ----------- 
    @Value("${file.upload-dir:}")
    private String configuredUploadRoot;
    // private String configuredUploadRoot;  // [251019] yml 키와 동일하게 수정
    ...
    /* ================================================================
    [251014 추가] 리치에디터 이미지 업로드 전용 API
    목적:
      - CMS 콘텐츠 폼(CmsContentForm.tsx)의 리치에디터에서 업로드된 이미지를 처리
      - react-draft-wysiwyg의 uploadCallback() 구조에 맞게 JSON 반환
    경로:
      - POST /api/files/upload/editor
    요청 파라미터:
      - image : MultipartFile
    반환 형식:
      { "data": { "link": "http://localhost:8181/images/editor/{uuid}.jpg" } }
    ================================================================ */
	@CrossOrigin("*")
	@PostMapping("/upload/editor")
	public ResponseEntity<Map<String, Object>> uploadEditorImage(@RequestParam("image") MultipartFile image) {
		try {
			// [1] 파일 저장 (FileServiceImpl.saveEditorImage 호출)
			String imageUrl = fileService.saveEditorImage(image);
			log.info("[251014] 리치에디터 이미지 업로드 성공: {}", imageUrl);
			...
			log.error("[251014] 리치에디터 이미지 업로드 실패: {}", e.getMessage());
