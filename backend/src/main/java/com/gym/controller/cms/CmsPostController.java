...
public class CmsPostController {
    ...
    public Long createPost(
			...
			@Parameter(name = "postType", description = "게시글 구분(selectbox: 일반/공지)", schema = @Schema(type = "string", allowableValues = {
					"일반", "공지" }, example = "일반")) @RequestParam(name = "postType", required = false) String postType,
			@Parameter(name = "postNotice", description = "공지 여부(true/false, 기본 false)", schema = @Schema(type = "string", allowableValues = {
					"true",
					"false" }, example = "false")) @RequestParam(name = "postNotice", required = false, defaultValue = "false") boolean postNotice,
			...
			@RequestParam(name = "postFilePath", required = false) String postFilePath // [251017] 첨부파일 경로 추가
    ) {
