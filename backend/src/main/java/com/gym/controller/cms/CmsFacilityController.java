...
public class CmsFacilityController {
    ...
    // -------------251025 시설삭제 기능 다시 활성화--------------
    /*
     해당 기능은 프론트엔드에선 활성화 안함
     이유: 시설정보가 삭제처리되면 예약건도 삭제되거나 전혀 다른 시설정보로 이동할 수 있음
     가장 좋은 방법은 시설을 완전히 삭제하는 대신 비활성화 처리임으로 삭제기능은 프론트 화면에선 활성화 X
     */
    @CrossOrigin("*")
    @Operation(
        summary = "시설 삭제(CMS)", 
        description = "PK 기준 시설 단건 삭제",
        // ⬇️ @Parameter 대신 @Operation의 parameters 속성을 사용하여 설명합니다.
        parameters = {
            @Parameter(name = "facilityId", description = "시설ID(PK)", required = true)
        }
    )
    @DeleteMapping("/{facilityId}")
    public ApiResponse<Void> deleteFacilityByIdCms(
        // @Parameter 제거
        @PathVariable("facilityId") Long facilityId
    ) {
        log.warn("[CMS][DELETE]/api/cms/facilities/{}", facilityId);
