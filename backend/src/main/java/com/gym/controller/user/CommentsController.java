...
@Tag(name = "13.Comment")
@RestController
//@RequestMapping("/api/posts/{postId}/comments") // /api/${boardId}/posts/${postId}/comments
@RequestMapping("/api/boards/{boardId}/posts/{postId}/comments") // [251020] 게시판ID로 명확하게 Mapping을 해야 함, 단순 api
// http://localhost:5173/api/boards/01/posts/25/comments 403 (Forbidden)
...
public class CommentsController {
	...
	public ResponseEntity<Long> createComments(@PathVariable("postId") Long postId,
			@RequestParam("commentContent") String commentContent) {
		log.info("[댓글 등록 요청] 게시글ID={}", postId);
		log.info("[댓글 등록 요청] 댓글내용={}", commentContent);
		...
		// DTO 생성
		CommentsCreateRequest req = new CommentsCreateRequest();
		req.setPostId(postId);
		req.setMemberId(loginId);
		req.setContent(commentContent);

		// 서비스 호출
		Long newId = commentsService.createComments(req);
		log.info("[댓글 등록 완료] 생성된 댓글ID={}", newId);
    ...
    public ResponseEntity<Void> updateComments(
            ...
            CommentsCreateRequest form // ✅ Form 전체 매핑 받기
    ) {
        ...
        // [251020] 도메인을 다이렉트로 사용
        form.setMemberId(loginId);
        commentsService.updateCommentsByMember(loginId, commentsId, form);
