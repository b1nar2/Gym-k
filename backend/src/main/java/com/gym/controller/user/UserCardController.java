...
public class UserCardController {
    ...
    */
    ...
    // ==============================[251009] 단건조회 대표 카드 및 나머지 카드 조회  ==============================
    /**
     * [251009] 내 카드 목록 조회 (토큰 기반)
     * - GET /api/cards
     * - 로그인된 회원ID 기준으로 card_tbl 조회
     */
    @CrossOrigin("*")
    @Operation(summary = "내 카드 목록", description = "로그인된 사용자의 카드 목록을 조회")
    @GetMapping("/api/cards")
    public ApiResponse<List<CardResponse>> getMyCards(Authentication auth) {
        String loginId = auth.getName(); // JWT로 인증된 사용자 ID
        log.info("[GET]/api/cards (본인조회) loginId={}", loginId);
        return ApiResponse.ok(cardService.listCardsByMember(loginId));
    }
    
    @CrossOrigin("*")
    @Operation(summary = "대표카드 조회", description = "로그인한 회원의 대표카드만 조회")
    @GetMapping("/api/cards/main")
    public ApiResponse<?> getMainCards(Authentication auth) {
        String memberId = auth.getName(); // ✅ 필터에서 로그인 ID 바로 추출
        log.info("[카드조회] 대표카드 조회 요청 - memberId: {}", memberId);
        return ApiResponse.ok(cardService.getMainCards(memberId));
    }

    @CrossOrigin("*")
    @Operation(summary = "일반카드 조회", description = "로그인한 회원의 대표가 아닌 카드 목록 조회")
    @GetMapping("/api/cards/sub")
    public ApiResponse<?> getSubCards(Authentication auth) {
        String memberId = auth.getName();
        log.info("[카드조회] 일반카드 조회 요청 - memberId: {}", memberId);
        return ApiResponse.ok(cardService.getSubCards(memberId));
    }
