...
public class UserAccountController {
    ...
    // ==============================[251009] 계좌 단건조회 대표계좌 및 나머지계좌 조회  ==============================
    
    /**
     * [251009] 내 계좌 목록 조회 (토큰 기반)
     * - GET /api/accounts
     * - 로그인된 회원ID 기준으로 account_tbl 조회
     */
    @CrossOrigin("*")
    @Operation(summary = "내 계좌 목록", description = "로그인된 사용자의 계좌 목록을 조회")
    @GetMapping("/api/accounts")
    public ApiResponse<List<AccountResponse>> getMyAccounts(Authentication auth) {
        String loginId = auth.getName(); // JWT로 인증된 사용자 ID
        log.info("[GET]/api/accounts (본인조회) loginId={}", loginId);
        return ApiResponse.ok(accountService.listAccountsByMember(loginId));
    }
    ...
    // ==============================[251009] 계좌단건조회 대표계좌 및 나머지계좌 단건조회  ==============================
