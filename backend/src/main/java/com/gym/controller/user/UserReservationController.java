...
public class UserReservationController {
    ...
    // ---------------------------------------------------------------------
    // [251027 신규] 특정 날짜 예약 가능 시간 조회
    @CrossOrigin("*")
    @Operation(summary = "특정 시설/날짜의 예약 완료 시간 목록 조회",
               description = "달력에 표시할 완료된 예약 시간대 목록을 반환합니다.")
    @GetMapping("/occupied-times")
    public ApiResponse<List<ReservationResponse>> getOccupiedTimes(
        @Parameter(description = "시설ID", required = true, schema = @Schema(type = "long"))
        @RequestParam("facilityId") Long facilityId,

        @Parameter(description = "신청일 (yyyy-MM-dd)", required = true, schema = @Schema(type = "string"))
        @RequestParam("wantDate") String wantDate
    ) {
        // 이 API는 보안상 민감하지 않으므로 인증(Authentication) 없이 사용 가능하도록 구성
        return ApiResponse.ok(reservationService.listCompletedReservationsByDate(facilityId, wantDate));
    }
    // ---------------------------------------------------------------------
