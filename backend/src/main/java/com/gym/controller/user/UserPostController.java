...
public class UserPostController {
    ...
    public Long createPost(
			...
			@Parameter(name = "postType", description = "게시글 구분(선택, selectbox: 일반만 허용)", schema = @Schema(type = "string", allowableValues = {
					"일반" }, example = "일반")) @RequestParam(name = "postType", required = false) String postType,
			@Parameter(name = "postSecret", description = "비밀글 여부(true/false, 기본 false)", schema = @Schema(type = "string", allowableValues = {
					"true",
					"false" }, example = "false")) @RequestParam(name = "postSecret", required = false, defaultValue = "false") boolean postSecret,
			//@Parameter(name = "postFilePath", required = false) String postFilePath // [251017] 첨부파일 경로 추가
			@Parameter(name = "postFilePath", required = false)// [251020] 첨부파일 경로 추가 @RequestParam 경로 전달되도록 설정
			@RequestParam(name = "postFilePath", required = false) String postFilePath // [251020]  
    ) {
        ...
        PostResponse req = PostResponse.builder()
                ...
                .postFilePath(postFilePath) // [251017] 첨부파일 경로 추가
                .build();
    ...
    public String updatePost(
            ...
            //@Parameter(name = "postFilePath", required = false) String postFilePath // [251017] 첨부파일 경로 추가
            @Parameter(name = "postFilePath", required = false)// [251020] 첨부파일 경로 
			@RequestParam(name = "postFilePath", required = false) String postFilePath // [251020] @RequestParam 경로 전달되도록 설정
    ) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication(); // 인증 주체
        if (auth == null || auth.getName() == null) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "로그인이 필요합니다."); // 401
