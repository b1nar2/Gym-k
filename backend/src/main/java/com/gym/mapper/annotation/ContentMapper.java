...
public interface ContentMapper {
    ...
    @Insert("""
        INSERT INTO contents_tbl (
            ...
            #{contentType},
            SYSDATE,
            #{contentFilePath, jdbcType=VARCHAR}
        )
    """)
    // [251013] 기준으로 content_file_path 기능 추가 
    ...
    @Select("""
        SELECT
          ...
          content_file_path  AS contentFilePath
        FROM contents_tbl
    ...
    @Update("""
        ...
        SET content_title    = #{contentTitle},
            ...
            content_file_path = #{contentFilePath, jdbcType=VARCHAR}
        WHERE content_id = #{contentId}
    """)
    ...
    /** [251012 추가] 정렬번호로 기존 콘텐츠 조회 (중복 메시지용) */
    @Select("""
        SELECT
          content_id       AS contentId,
          content_title    AS contentTitle,
    ...
    """)
    ContentResponse findByContentNum(@Param("contentNum") Integer contentNum);
