    ...
    */
    ...
    @Select("""
        SELECT
            ...
            account_main     AS accountMain, -- alias 동일
            account_reg_date AS accountRegDate
        FROM account_tbl
        WHERE member_id = #{memberId}
          AND account_main = 'Y'
    """)
    @Results({
    	@Result(column="account_main", property="accountMain", // ✅ DB 컬럼명으로 고쳐야 함
                typeHandler=BooleanYNTypeHandler.class)
    })
    List<Account> selectMainAccounts(@Param("memberId") String memberId);


    // 일반계좌만 조회 (account_main = 'N')
    @Select("""
        SELECT
            account_id       AS accountId,
            member_id        AS memberId,
    ...
    @Results({
        @Result(column="account_main", property="accountMain",
        		typeHandler=BooleanYNTypeHandler.class) // ✅ 여기에만 선언
    })
    List<Account> selectSubAccounts(@Param("memberId") String memberId);
