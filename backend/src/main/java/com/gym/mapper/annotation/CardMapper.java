...
public interface CardMapper {
    ...
    // #{cardApproval} -> #{cardApproval, jdbcType=VARCHAR} 이유 카드 등록시 null값으로도 승인되게끔 하기 위해서
    @Insert("""
    ...
    */
    ...
    @Select("""
        SELECT
            ...
            card_approval  AS cardApproval,
            card_main      AS cardMain,
            card_reg_date  AS cardRegDate
        FROM card_tbl
        WHERE member_id = #{memberId}
          AND card_main = 'Y'
    """)
    @Results({
        @Result(column="card_main", property="cardMain",
                typeHandler=com.gym.config.type.BooleanYNTypeHandler.class)
    })
    List<Card> selectMainCards(@Param("memberId") String memberId);
    ...
    List<Card> selectSubCards(@Param("memberId") String memberId);
}
