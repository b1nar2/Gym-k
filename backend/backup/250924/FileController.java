...
public class FileController {
    ...
    */
    ...
    // ---------------------------------------------------------------------
    // 3) 파일 미리보기 — /files/{filename}/preview (inline)
    // ---------------------------------------------------------------------
    
    @Operation(summary = "파일 미리보기", description = "브라우저에서 바로 열 수 있도록 inline으로 바이너리 응답합니다.")
    @GetMapping(value = "/{filename}/preview")
    public ResponseEntity<InputStreamResource> preview(
            @Parameter(description = "파일명(확장자 포함)", required = true)
            @PathVariable("filename") String filename
    ) throws IOException {

        // (1) DB에서 파일명으로 경로/메타 조회
        FileResponse fileInfo = fileQueryMapper.selectFileByName(filename);
    ...
    /*
    @Operation(summary = "파일 미리보기(파일번호)", description = "파일 PK(file_id)로 브라우저에서 inline 미리보기합니다.")
    @GetMapping(value = "/id/{fileId}/preview")
    public ResponseEntity<InputStreamResource> previewById(
            @Parameter(description = "파일 PK(file_id)", required = true)
            @PathVariable("fileId") Long fileId
    ) throws IOException {

        // (1) DB에서 파일 PK로 경로/메타 조회 (filename 버전과 동일한 처리 흐름)
        FileResponse fileInfo = fileQueryMapper.selectFileById(fileId);  // ⬅️ 조회만 변경
        if (fileInfo == null || fileInfo.getFilePath() == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }

        // (2) 실제 경로에서 파일 스트림 열기 (동일)
        Path path = Paths.get(fileInfo.getFilePath());
        if (!Files.exists(path)) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
    ...
    */
    ...
    @Operation(
    	    summary = "파일 다운로드(파일명으로 조회)",
    	    description = "DB의 file_tbl에서 파일명을 검색해, 저장 경로(file_path)의 실제 파일을 첨부로 내려줍니다."
    	)
    	@GetMapping(value = "/download")
    	public void downloadByName(
    	        HttpServletRequest request,
    	        HttpServletResponse response,
    	        @Parameter(description = "다운로드할 원본 파일명(예: 2025-09-15 업무정리.txt)", required = true)
    	    ...
    	    // 1) DB 조회: 최신 1건
    	    FileResponse file = fileQueryMapper.selectFileByName(fileName);
    ...
    /*
    @Operation(
    	    summary = "파일 다운로드(파일번호)",
    	    description = "file_tbl의 PK(file_id)로 조회해 실제 파일을 첨부로 내려줍니다."
    	)
    	...
    	public void downloadById(
    	    ...
    	    // 0) 로그기록 (패턴 동일)
    	    log.info("다운로드 파일ID:{}", fileId);

    	    // 1) DB 조회: 단건(PK)
    	    FileResponse file = fileQueryMapper.selectFileById(fileId);   // ⬅️ 조회만 변경
    	    if (file == null || file.getFilePath() == null) {
    	        response.setStatus(HttpServletResponse.SC_NOT_FOUND);
    	        return;
    	    }
