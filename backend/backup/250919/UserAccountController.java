 ...
 * - 요구사항: JSON → 입력폼(form-urlencoded) 전환, 본인 소유만 허용
 * - 기존(JSON) 메서드는 [old] 주석으로 비활성화
 */
...
public class UserAccountController {
    ...
    private final AccountService accountService;        // 서비스 주입
     ...
     * - 목적: memberId 입력폼 제거, 작성자ID는 로그인ID로 자동 설정
     * - 전제: SecurityConfig에서 본 엔드포인트는 인증 필요로 보호됨(미인증 시 필터에서 401/403)
     * - 매퍼/서비스/DDL 변경 없음
     */
    @Operation(
        summary = "계좌 등록",
        description = "account_tbl INSERT (폼 입력, 작성자ID는 로그인ID로 자동 설정)"
    )
    ...
    public ApiResponse<Long> createAccountForm(
            ...
            @RequestParam("accountNumber") String accountNumber,                  // 필수 입력: 계좌번호
        ...
        // 작성자ID는 로그인ID로 고정(폼으로 받지 않음)
        final String loginId = auth.getName(); // 시큐리티 컨텍스트 기반으로 스푸핑 불가
        ...
        req.setAccountMain(accountMain);     // 대표여부
    ...
    @Operation(summary = "회원별 계좌 목록", description = "memberId 기준 SELECT (본인만 조회 가능)")
    @GetMapping("/api/members/{memberId}/accounts")
    public ApiResponse<List<AccountResponse>> listByMember(
            @Parameter(description = "회원ID") @PathVariable("memberId") String memberId,
            Authentication auth
     ...
     * - 목적: memberId 입력폼 제거, 로그인ID로 고정하여 본인 계좌만 대표로 설정
     * - 전제: 보안 설정으로 미인증 호출은 필터에서 차단(컨트롤러 진입 전 401/403)
     * - 매퍼/서비스/DDL 변경 없음
     */
    @Operation(summary = "대표계좌 설정", description = "대상만 'Y', 나머지 자동 'N' (본인만 가능)")
    @PatchMapping("/api/accounts/{accountId}/main")
    public ApiResponse<Void> setMainAccount(
    ...
    @Operation(summary = "계좌 삭제", description = "PK로 단건 삭제(본인 소유만 허용, 대표계좌 트리거 주의)")
    @DeleteMapping("/api/accounts/{accountId}")
    public ApiResponse<Void> deleteAccountById(
            @Parameter(description = "삭제할 계좌 PK") @PathVariable("accountId") Long accountId,
            Authentication auth
