...
-- [251013] 예약 상태 변경 시 결제 상태 동기화 트리거
CREATE OR REPLACE TRIGGER trg_reservation_to_payment
AFTER UPDATE OF resv_status ON reservation_tbl
FOR EACH ROW
DECLARE
BEGIN
  -- 예약이 완료되면 결제도 완료 처리
  IF :NEW.resv_status = '완료' 
     AND NVL(:OLD.resv_status,'§') <> '완료' THEN
    UPDATE payment_tbl
       SET payment_status = '완료'
     WHERE resv_id = :NEW.resv_id
       AND payment_status = '예약';  -- 기존 대기 상태만 갱신

  -- 예약이 취소되면 결제도 취소 처리
  ELSIF :NEW.resv_status = '취소'
        AND NVL(:OLD.resv_status,'§') <> '취소' THEN
    UPDATE payment_tbl
       SET payment_status = '취소'
     WHERE resv_id = :NEW.resv_id
       AND payment_status <> '취소'; -- 이미 취소된 건 제외
  END IF;
END;
/
ALTER TRIGGER trg_reservation_to_payment ENABLE;
