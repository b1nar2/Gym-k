...
-- 251028 변경 후 제약조건 (contentType, contentNum 복합)
BEGIN
EXECUTE IMMEDIATE '
...
----------------------------------------------------------
-- [251013 수정본] 콘텐츠 첨부파일 경로 컬럼 추가
----------------------------------------------------------
ALTER TABLE contents_tbl ADD content_file_path VARCHAR2(300);
COMMENT ON COLUMN contents_tbl.content_file_path IS '콘텐츠 첨부파일 경로(썸네일/문서 등)';
----------------------------------------------------------

-------------------------------------------------------------------------------
-- [251023 수정본] '상품·시설안내' → '상품/시설안내'로 변경하기
-------------------------------------------------------------------------------
-- 1. 테이블 내 모든 '상품/시설안내'를 '상품·시설안내'로 변경
UPDATE contents_tbl
SET content_type = '상품·시설안내'
WHERE content_type = '상품/시설안내';

COMMIT;
-- 변경된 데이터 확인: '상품/시설안내'는 0건이어야 함
SELECT content_type, COUNT(*) cnt
FROM contents_tbl
GROUP BY content_type;

-- 2-1. 기존 제약 조건 삭제
BEGIN
EXECUTE IMMEDIATE 'ALTER TABLE contents_tbl DROP CONSTRAINT content_type_ch';
EXCEPTION
WHEN OTHERS THEN
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다.
-- ORA-02443: 제약 조건이 존재하지 않습니다. (제약조건이 이미 없으면 무시)
IF SQLCODE NOT IN (-942, -2443, -20000) THEN
RAISE;
END IF;
END;
/
...
-- 2-2. 제약 조건 변경하기
ALTER TABLE contents_tbl ADD CONSTRAINT content_type_ch 
CHECK (content_type IN ('이용안내','상품·시설안내'));

-- 2-3. 새로운 제약조건 확인
SELECT constraint_name, constraint_type, status, search_condition
FROM user_constraints
WHERE table_name = 'CONTENTS_TBL'
AND constraint_name = 'CONTENT_TYPE_CH';

-- 콘텐츠 테이블의 content_type 값 최종 확인
SELECT DISTINCT content_type FROM contents_tbl;
-------------------------------------------------------------------------------
