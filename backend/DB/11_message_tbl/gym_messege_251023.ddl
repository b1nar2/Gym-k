...
--------------------------------------------------------------------------------
-- [251016 컬럼 추가] 메시지 읽음 상태를 나타내는 read_status 컬럼을 추가
-- 한 글자의 문자 데이터 타입, 기본값은 'N'(읽지 않음), NULL이 될 수 없음.

ALTER TABLE message_tbl ADD (
  read_status CHAR(1) DEFAULT 'N' NOT NULL
);
...
--[251016 추가]
COMMENT ON COLUMN message_tbl.read_status     IS '메시지 읽음 상태';
...
-- 251017 추가 메시지 읽음 처리(CHECK)
ALTER TABLE message_tbl ADD CONSTRAINT read_status_CH
CHECK (read_status IN ('N', 'Y'));
...
INSERT INTO message_tbl (
  ...
  seq_message_id.NEXTVAL, 'hong1', NULL, NULL, '휴관공지',
  'hong1 테스트.'
);
...
-- 251020 읽음 처리 확인
SELECT message_id, read_status
FROM message_tbl
WHERE message_id = 13; 

UPDATE message_tbl
SET read_status = 'Y'
WHERE message_id = 13; -- 테스트할 메시지 ID
